  ...
  return declare([_WidgetBase, _TemplatedMixin], {
    ...
    addLayerNode: function(layerInfo, level, toTableNode) {
      var layerTrNode = domConstruct.create('tr', {
        ...
        popupMenuNode, i, imageShowLegendDiv, popupMenu, divLabel, group;
      ...
      */
      ...
      if(this.config.layerOptions && this.config.layerOptions[layerInfo.id] !== undefined) {
        ...
        input_val = this.config.layerOptions[layerInfo.id].group;
      }
      ...
      */
      ...
      //my addition 
      if (level == 0){
    	  grpLable = domConstruct.create("label", {	                    
	          'style': "",
	          'innerHTML':"Enter group for this layer: "
	       }, layerTdNode);      
	      group = domConstruct.create("input", {
	          'id': layerInfo.id,          
	          'type': "text",
	          'value': input_val
	       }, layerTdNode);      
      ...
      this.initDisplayStateStorage(layerInfo, ckSelect, divLabel, group);
   ...
   /* _oninputNodeChange: function(layerInfo, group, evt){
    ...
    initDisplayStateStorage: function(layerInfo, displayCK, labelDiv, group) {
    ...
    getLayerOptions: function() {
      ...
      for(var child in this._displayStateStorage) {
        if(this._displayStateStorage.hasOwnProperty(child) &&
        	...
        	//console.warn('gp', this._displayStateStorage[child].group);
        	if(this._displayStateStorage[child].group !== undefined){
          layerOptions[child] = {display: this._displayStateStorage[child].displayCK.checked, group: this._displayStateStorage[child].group.value};
        	}else{
